...
class CarBase(BaseModel):
    ...
    current_speed: float = Field(default=0.0, ge=0.0, le=50.0, description="当前速度(km/h)")
    ...
    battery_level: float = Field(default=100.0, ge=0.0, le=100.0, description="电量百分比")
    ...
    @classmethod
    def validate_battery_level(cls, v: float) -> float:
        """验证电量百分比"""
        if v < 0 or v > 100:
